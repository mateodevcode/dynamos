...
export default function Titulares() {
  ...
  return (
    <div className="relative flex flex-col w-fit max-w-4xl mx-auto bg-[#eca058]">
      <div className="flex flex-col custom:flex-row">
        ...
        <div className="flex flex-col justify-between py-4 px-8">
          <div>
            ...
            <p className={`custom:w-[15rem] text-justify text-sm ${fadeIn ? 'opacity-100' : 'opacity-0'}`}>Una carta es flex-col un medio de comunicación escrita por un remitente y enviado a un destinatario. Habitualmente, el nombre y la dirección del destinatario aparecen en el frente del sobre, el nombre y la dirección del remitente aparecen en el reverso del mismo o en el anverso. </p>
          </div>
          <div className='flex gap-3 w-fit mx-auto mt-6'>
            {titulares.map((titular) => (
              <div
                ...
                key={titular.id}
                className={`w-3 h-3 rounded-full transition-all cursor-pointer hover:scale-125 active:scale-100 ${titular.id === noticia.id ? "bg-teal-800" : "bg-white"}`}
              ></div>
            ))}
          </div>
        </div>
